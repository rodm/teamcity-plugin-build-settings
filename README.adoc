...
* `vcs.branches` - A comma separated list of additional branches to monitor and build. The default is to monitor
`vcs.branch` and tags.
...
* `java.home` - A path or configuration parameter that defines a path to a JDK installation. The default is `%java21.home`.
